    ...
    def process(self):
        ...
        for filename in self.files:
            ...
            try:
                ...
                place_dict = geoparse_obj['places_by_entityURI']
                places[source] = geoparse_obj['places_by_entityURI'
                        ][source]
                for target in geoparse_obj.get('places'):
                    if target in validEntityURIs:
                        places[target] = place_dict[target]
        ...
        for year in years:
            ...
            for (edge, text_weights) in linksByYear[year].iteritems():
                ...
                groupedLinksByYear[year].append({
                    'edge': [source, target],
                    'texts': texts,
                    'year': year,
                    'weight': weight,
        ...
        params = {
            'STARTDATE': min(linksByYear.keys()),
