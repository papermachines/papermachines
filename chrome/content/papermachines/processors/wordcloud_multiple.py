...
class MultipleWordClouds(wordcloud.WordCloud):
        ...
        for i in xrange(1, n):
            if svec[i] == last_obs:
                ...
                if len(dupe_indices) > 0:
                    averank = sum(dupe_indices) \
                        / float(len(dupe_indices)) + 1
                    ...
                    new_vec[i] = i + 1
            last_obs = svec[i]
        ranks = dict((svec[i], rank) for (i, rank) in
                     enumerate(new_vec))
        return ranks
    ...
    def _dunning_held_out(
        ...
        for i in range(len(sets)):
            for filename in sets[i]:
                ...
                count_total[i + 2] += \
                    sum(self.tf_by_doc[filename].values())
    ...
    def _dunning(self, word, label_set):
        count_total = [0.0, self.freqs[word], 0.0,
        ...
        for label in self.label_order:
            ...
            if self.tfidf_scoring and self.MWW:
                ...
                for word in self.top_tfidf_words:
                    word_rho[word] = self._held_out(word, label_set,
                            other_set)
                ...
                for filename in filenames:
                    for (term, weight) in \
                        self.tf_by_doc[filename].iteritems():
                        if term not in tf_maxes:
                            tf_maxes[term] = weight
                        else:
                            if weight > tf_maxes[term]:
                                tf_maxes[term] = weight
                tfidf_for_labelset = dict((term, weight
                        * self.idf[term]) for (term, weight) in
                        tf_maxes.iteritems())
                filtered_freqs_for_labelset = dict((term, freq)
                        for (term, freq) in
                        self.filtered_freqs.iteritems() if term
                        in tfidf_for_labelset)
